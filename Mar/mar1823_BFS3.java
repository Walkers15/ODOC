...
public class mar1823_BFS3 {
  public static void main(String[] args) throws IOException {
    ...
    Queue<Info> afterBreakQueue = new LinkedList<>();
    ...
    while (!queue.isEmpty()) {
      ...
      for (int i = 0; i < 4; i++) {
        ...
        if (inRange && !visited[y][x]) {
          ...
          if (map[y][x] == 1) {
            if (!wallBroke[y][x] && time < result) {
              // System.out.printf("Break Wall %d %d in time %d\n", x, y, time);
              wallBroke[y][x] = true;
              // showVisited(visited);
              afterBreakQueue.add(new Info(x, y, time));
            }
          }
        }
      }
    ...
    while (!afterBreakQueue.isEmpty()) {
      Info info = afterBreakQueue.poll();
      if (info.time > result) {
        continue;
      }
