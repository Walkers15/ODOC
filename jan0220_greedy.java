        ...
        for(int i = 0 ; i < bc ; i++) {
            ...
            for(int j = arr.get(i).from-1 ; j < arr.get(i).to-1 ; j++){
                if(Capacity - dp[j] < arr.get(i).count){//이번 박스의 양이 현재 적재할 수 있는 양보다 큰 경우
                    if(min > Capacity-dp[j]) min = Capacity-dp[j];//이번 박스에서 쪼개서 넣는 양을, 남은 트럭의 용량만큼 채워줌
                }
            }
...
class box implements Comparable<box>{
    ...
    public int compareTo(box B){
        if(this.to > B.to) return 1;
        else if(this.to == B.to){
            if(this.from > B.from) return 1;
            else if(this.from < B.from) return -1;
            return 0;
        }
        else return -1;
    }
